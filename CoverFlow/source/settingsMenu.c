...
#ifdef NEWS_READER
					else if (Button_Select(&checkNewsButton, pointer.p_x, pointer.p_y))
					{
					 if (getNewsFile())
					 	{
						 WindowPrompt( "CFNS", "New Coverfloader-News available!", &okButton, 0);
						 self.blinkBlue = true;
						}
					}
#endif					
	...
	/////////////////////////////////////////////////////////
			{
				...
				CFreeTypeGX_DrawText(ttf16pt, 320, 450, "Covers courtesy of muntrue.nl, wiitdb.com & wiiboxart.com", (GXColor){0x44, 0x44, 0x44, 0xff}, FTGX_JUSTIFY_CENTER);
				// Check for button-pointer intersections, and rumble
				if (Button_Hover(&menuSettingsButton, pointer.p_x, pointer.p_y) ||
					Button_Hover(&menuGraphicsButton, pointer.p_x, pointer.p_y) ||
					Button_Hover(&menuAdvancedButton, pointer.p_x, pointer.p_y) ||
