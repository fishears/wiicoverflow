...
void parse_bns(void *data_bns, SoundInfo *snd)
{
	...
	//dbg4_printf("BNS: %.4s 0x%x %x %d\n", bns->bns, bns->version, bns->filesize, bns->chunkcount);
...
//				i_chunk->offset);
	...
	dsp_data = CFMemAlign(32, dsp_size + 14*2);
...
//			dbg4_printf("%d %d\n", ssnd->offset, ssnd->bsize);
	...
	void *dsp_data = CFMemAlign(32, dsp_size);
	if (!dsp_data) return;
