...
s32 Fat_MountWBFS(u32 sector)
{
	...
	if (wbfsDev == WBFS_DEVICE_USB) {
		...
		ret = __io_usbstorage.startup();
		if (!ret) {
			printf(gt("ERROR: USB init! (%d)"), ret); printf("\n"); sleep(1);
			return -1;
		}
		/* Mount device */
		ret = fatMount(WBFS_MOUNT, &__io_usbstorage, sector, FAT_CACHE_SECTORS);
		if (!ret) {
			printf(gt("ERROR: USB/FAT mount wbfs! (%d)"), ret); printf("\n"); sleep(1);
			return -2;
		}
		...
		if (!ret) {
			printf(gt("ERROR: SD/FAT mount wbfs! (%d)"), ret); printf("\n"); sleep(1);
			return -5;
		}
	}
...
s32 MountNTFS(u32 sector)
{
	...
	if (wbfsDev == WBFS_DEVICE_USB) {
		...
		if (!ret) {
			printf(gt("ERROR: USB mount NTFS! (%d)"), ret);
			printf("(%d)\n", errno);
			sleep(2);
			return -2;
		}
		...
		if (!ret) {
			printf(gt("ERROR: SD mount NTFS! (%d)"), ret); printf("\n"); sleep(1);
			return -5;
		}
	}
...
void _FAT_mem_init()
{
	...
	fat_pool = CFMemAlign(32, fat_size);
}
