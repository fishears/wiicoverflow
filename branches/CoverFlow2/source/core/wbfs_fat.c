...
/*XFATX if we use this we'll have to rewrite it
...
s32 WBFS_FAT_DiskSpace(f32 *used, f32 *free)
{
	...
	*free = 0;
		...
		|| wbfs_fat_vfs_dev != wbfsDev )
	{
		...
		printf("\r");
		printf(gt("calculating space, please wait..."));
		printf("\r");
		//dbg_time1();
		//dbg_printf("\nstatvfs(%s)\n", wbfs_fs_drive);
...
s32 WBFS_FAT_RemoveGame(u8 *discid)
{
	...
	//rmdir(path);
	if (unlink(path) == -1) {
		printf(gt("ERROR removing %s"), path);
		printf("\n");
