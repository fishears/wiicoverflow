...
/* XFATX
    if (settings.cios == ios222 && IOS_GetVersion() != 222) 
		{
			Fat_UnmountSDHC();	// unmount SD for reloading IOS
			Fat_UnmountUSB();	// unmount USB for reloading IOS
			...
			Fat_MountSDHC(); 		// now mount SD:/
			Fat_MountUSB(); 	// and mount USB:/
		} 
		else if (settings.cios == ios249 && IOS_GetVersion() != 249) 
		{
			Fat_MountSDHC();	// unmount SD for reloading IOS
			Fat_MountUSB();	// unmount USB for reloading IOS
			ret = IOS_ReloadIOS(249);
			if (ret < 0) 
			{
...
//////////////////////////////////////////////////////////////////////				    
								...
								//Fat_UnmountSDHC();    /* cheats aren't working */
								//Fat_UnmountUSB();
